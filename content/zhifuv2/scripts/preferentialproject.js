    ...
    */
    chooseCoupons: function (e, couponsCode) {
        ...
        if ($(e).attr("class") == "sale-coupon sale-coupon-hover") {
            ...
            hidVoucher.val(selectedCoupons.code).attr("price", selectedCoupons.price);
...
var PreferentialProject = {
    ...
    setChecked: function () {
        ...
        for (var i = 0; i < preferentialIds.length; i++) {
            $("input[type='checkbox'][id='" + preferentialIds[i] + "']").parent().find("label").attr("disabled", "disabled").attr("class", "input-checkbox no_select");
            $("input[type='checkbox'][id='" + preferentialIds[i] + "']").attr("disabled", "disabled");
        }
        return result;
    },
    // 选择优惠活动
    ...
    clearactivitySelect: function (type) {
        if (type == 1) {
            ...
            if (CheckBankPreferentialId > 0) {
                $("#li" + CheckBankPreferentialId).click();
                $("#CheckBankPreferentialId").val(0);
            } else {
                $(".bankname").eq(0).click();
            }
        } else {
            $(".bank_card_top").find("li:eq(0)").click();
            $(".bank_card_m").find(".bank_card_m1_img:eq(0)").find("li:eq(0)").click();
            PreferentialProject.selectedActivities = [];
            var checked = $("input[type='checkbox']").attr("checked", false);
            ...
            if (IsUsePreferential == 1) {
                ...
                //默认选中最大的优惠
                $("input[type='checkbox'][name*='preferential_']").parent().find("label").click();
            }
            // 重新生成预订清单
            renderBooklist();
            // 将已选择的优惠活动写入隐藏域
