...
* 2016-12-16  潘振兴   添加宝钢优惠券互斥规则
* 2017-4-11   潘振兴   添加国航优惠券互斥规则
*/
...
var MemberPoints = {
    usePoints: function (e) {
        ...
        var errorCtl = o.parent().next(".cf30");
        errorCtl.text("");
                ...
                if (score > 0) {
                    if (score <= count) {
                        if (score % ratio == 0) {
                            ...
                            lis.each(function () {
                                $(this).find("input[type='checkbox'],label").hide();
                                if ($(this).find("span")) {
                                    $(this).find("span").remove();
                                }
                                $(this).append("<span>不与" + "其他优惠" + "同享</span>");
                            });
                        }
                        else {
...
var Coupons = {
    ...
    */
    chooseCoupons: function (e, couponsCode) {
        // 如果已经选中
        if ($(e).attr("class") == "sale-coupon sale-coupon-hover") {
            ...
            // 再将当前选中的券置为选中状态
            $(e).attr("class", "sale-coupon sale-coupon-hover");
            ...
            $('#txtUseVoucher').removeAttr("disabled");
            $('#txtUseVoucher').val("");
            $('#btnUseVoucher').text('使用');
            $('#couponsTip').html('');
            ...
            lis.each(function () {
                ...
                $(this).append("<span>不与" + "抵用券/红包" + "同享</span>");
            });
            ...
            if (PreferentialProject.selectedCoupons != null && (PreferentialProject.selectedCoupons.suppelierID == 35 || PreferentialProject.selectedCoupons.suppelierID == 40)) { //宝钢优惠券-去掉已选中的标准优惠,标记为不能与宝钢优惠同享
                ...
                var lis = $("input[name='preferential_" + 3 + "']").parent();
                lis.each(function () {

                    //去掉选中的标准优惠
                    $(this).find("input[type='checkbox']:checked").each(function (i) {
                        var id = $(this).attr("id");
                        PreferentialProject.activitySelect($("label[for='" + id + "']"), id, 3);
                    ...
                    $(this).append("<span>不与" + couponsTitle + "同享</span>");
                });
            } else {//不是宝钢优惠设置初始状态
                // 大类互斥检测
                if (PreferentialProject.jsonData) {
    ...
    */
    useCoupons: function (e) {
        ...
        if (o.text() == "使用") {
            ...
            if (!v) {
                o.removeAttr("disabled");
                //errorCtl.text("请输入要使用的券号");
                showError($('#txtUseVoucher'), "请输入要使用的券号");
                $('#txtUseVoucher').val('');
                $('#txtUseVoucher').focus();
                return;
            }
            ...
            $.ajax({
                ...
                success: function (msg) {
                    ...
                    if (parseInt(msg) > 0) {//验证成功
                        ...
                        $('.coupons-details input:radio').attr("disabled", true);
                        $('#txtUseVoucher').removeClass('error');
                        o.text("取消");
                        txtVoucher.prop("disabled", true);
                        $('#couponsTip').html("券一旦使用，生成订单后，将无法退回。");
            ...
            o.text("使用");
            $('#txtUseVoucher').val('');
            txtVoucher.prop("disabled", false);
    ...
    categoryMutexCheck: function (categoryId) {
        if (this.selectedActivities.length == 0) {
            for (var i = 0; i < this.jsonData.Preferentials.length; i++) {
                ...
                var lis = $("input[name='preferential_" + preferentialModel.CategoryID + "']").parent();
                lis.each(function () {
                    $(this).find("input[type='checkbox'],label").show();
                    $(this).find("span").remove();
                });
                    ...
                    if (mutexeCategorieCodes.length > 0) {
                        ...
                        lis.each(function () {
                            ...
                            if (preferentialModel.CategoryID == 1) {
                                ...
                                $(this).append("<span>不与" + textArr.join(",") + "同享</span>");
                            }
                        });
                    }
                }
        ...
        if (categoryId == 1) {//1=银行优惠
            ...
            var lis = $("input[name='preferential_" + categoryId + "']").parent(); //银行大类下的优惠
            if (selectedBankActivityId) {
                lis.each(function () {
                    var currentVal = $(this).find("input[type='checkbox']").val();
            ...
            if (preferentialModel.InnerMutex == 0) { //类内互斥
                ...
                if (isLogin) {
                    ...
                    if (preferentialModel.CategoryID == 1) {
                        // 银行不与积分、优惠券同享
                        $('#point').hide();
                        $('#pointDisabledTip').html('不与银行优惠同享');
                        $('#pointDisabled').show();

                        $('#couponsDisabled').html('不与银行优惠同享').show();
                        $('#couponsContent').hide();
                    }
                    else if (preferentialModel.CategoryID == 2) {
                        // 立减不与优惠券同享(优惠券属于立减类优惠，而立减类内互斥)
                        $('#couponsDisabled').html('不与立减优惠同享').show();
                        $('#couponsContent').hide();
                    }
                }
            }
