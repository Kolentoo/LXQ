...
function IsSale()
{
    ...
    var uztype = $('#hidUztype').val();//链接来源
    $.ajax({
        type: 'GET',
        url: 'http://sh.uzai.com/ashx/ashx_Calendar.ashx?pid=' + pid + '&type=1&uztype=' + uztype+"&isSale=1",
        dataType: 'jsonp',
        success: function (d) {
           if(d[0]===0)
           {
               setTimeout(function(){
                   $(".J_uzaiprice .price-mod").addClass("orange vm").html('<i class="price"><em class="f20">请电询</em></i>');
                   $(".order-box-ft").find("a").eq(0).attr("style", "filter: Alpha(opacity=40); -moz-opacity: .1; opacity: 0.4;");
                   $(".order-box-ft").find("a").eq(0).attr("disabled", "disabled");
                   $(".order-box-ft").find("a").eq(0).removeAttr("onclick");
               },1000);
...
function commentRedirect() {
    window.location.href = _uzw.domain.u + "/manage/tour-record";
}
...
function commentPop() {
    ...
    $('#j_btnRemark').on('click', function () {
        if (!_uzw.user.userid) {
            _uzw.iframe.pop(_uzw.domain.u + '/QuickLoginV1?actionName=commentRedirect');
        } else {
            //国双统计代码【我要点评】
...
function latestOrderScroll() {
    ...
    $.ajax({
        type: "GET",
        url: "/ashx/ashx_LvyoucnNewBooking.ashx?pid=" + id,
        dataType: "json",
...
// 从优惠名称中获取金额 by gaochong 2016-12-22
var getActivityPrice = function (displayName) {
    var _price = 0;

    var priceStr = displayName.match(/\d+元\//g);//以"300元/"匹配
    if (priceStr) {
        var price = priceStr[0].match(/\d+/g);//从匹配到的字符串中取出优惠金额
        if (price) {
            _price = parseInt(price[0], 10);
        }
    }
...
// 处理优惠名称，如果带金额，则金额变成红色 by gaochong 2016-12-22
var getDisplayNameWithSpan = function(displayName){
   /* var _price = getActivityPrice(displayName);
    if (_price > 0) {
        var displayNameNew = displayName.replace(/\d+元/g, '<b>' + _price + '元</b>');
        return displayNameNew;
    }
    else {
        return displayName;
    }*/
   var displayNameNew = displayName.replace(/元/g,'元</b>').replace(/减/g,'减<b>');
   return displayNameNew;
};
...
var yhTips = function (block, tipBox) {
    ...
    var tags = function (obj) {
        ...
        if (obj.get(0)) {
            ...
            */
            // UI改版，by gaochong 2016-12-22 优惠二期改动
            if (item) {
                var html = "<div class='clearfix item'><div class='fl mr10'><i>优惠</i></div>";
                html += '<div class="f1" style="float: left">';
                for (var i = 0; i < item.length; i++) {
                    ...
                    html += '<p>' + getDisplayNameWithSpan(desc) + '</p>';
    ...
    if (eDate) {
        ...
        var oldTime = new Date(eDate + " " + eTime);
        if (oldTime > currTime) { sb.push("<p>" + eDate + aop + eTime + "截止收取签证材料</p>"); }
    }
...
function smCalendar() {
    ...
    var uztype = $('#hidUztype').val();
    var cfg = {
        jsonpUrl: 'http://bj.uzai.com/ashx/ashx_Calendar.ashx?pid=' + pid + '&type=1' + "&uztype=" + uztype,
        isSmart: true,
        latestDate: firstDay,
                    ...
                    if (oa.get(0)) {
                        ...
                        o.on('click', function () {
                            if (isGenTuan) {
						        ...
						        ajaxImmCutCheck($("#pid").val(), date, $("#hidSupplierFlag").val());
...
function startDate() {
    ...
    oSD.on('click', 'li', function () {
        ...
        ajaxImmCutCheck($("#pid").val(), odate, $("#hidSupplierFlag").val());
...
function productInterest(type) {
    if (_uzw.user.userid) {
        var paras = {
            ...
            "type": type, //来源类型：1浏览历史、2我的收藏、3我的咨询、4意向订单、5支付订单、6关键词搜索、7自主点击不喜欢
...
function askPop(){
	//点击咨询
    $('#j_btnAsking_new').on('click', function () {
		//显示咨询输入层
		$('.advisory-mod-bd .pop-problem').show();
		//隐藏按钮
		$('.pop-problem .pop-btn').find('.p2').unbind('click'); //避免重复绑定
		$('.pop-problem .pop-btn').find('.p2').on('click', function () {
			$('.advisory-mod-bd .pop-problem').hide();
		    //清空表单
	    	$('#askCont').val('');
	    	$('#phone').val('');
	    	$('#email').val('');
		...
		$('.pop-problem .pop-btn .p1').find('.btn-txt1').unbind('click'); //避免重复绑定
		$('.pop-problem .pop-btn .p1').find('.btn-txt1').on('click', function () {
			productFeedBack();
		});
    });
    //快速登录
    $('.advisory-mod-bd .advisory-tips .login').on('click', function () {
    ...
    //根据用户登录状态判断是否显示提示
    if(_uzw.user.userid)
        $('.advisory-mod-bd .advisory-tips span').remove();
...
function productFeedBack(){
    ...
    var checkMsgValidate = function () {
		...
		if (o3.length > 0 && !isValidateEmail) {
    ...
    if (checkMsgValidate()) {
    	$(".error-info").text('');
    	$(".pop-problem .pop-btn .p1 .btn-mask").show(); //不可点击
            ...
            success: function (data) {
                if (data) {
                    ...
                    if (msg.result == "success") {
                        ...
                        $('.pop-problem .pop-btn').find('.p2').click();
                    }
...
//咨询数据加载 新旧版点评通用 productZixun ，有些旧版专用代码
function productZiXun() {
    var pcount = parseInt($('#j_zuxunCount').text(), 10);
    pcount = pcount || 0;
    if (pcount === 0) {
        ...
        df.done(function (msg) {
            ...
            if (msg) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    sb.push("<div class='advisory-bd pt10 clearfix'>");
                    sb.push("<div class='item-hd fl'><i class='icon-answer mr10 vm icon-product png'>&nbsp;</i><span class='hd-cont'>悠哉客服：</span></div>");
                    if(aContent.length > 65){
	                    sb.push("<div class='item-bd' style='width:800px;'><em class='answer-cont'>" + aContent.substring(0, 65) + "</em><em class='answer-cont hide'>" + aContent + "</em></div>");
	                    sb.push("<div class='sela fl'><a href='javascript:void(0);' onclick='showDetail(this)'>查看完整回复</a></div>");
                    } else {
                    	sb.push("<div class='item-bd' style='width:800px;'><em class='answer-cont'>" + aContent + "</em></div>");
                    }
                    sb.push("</div>");
                    sb.push("</li>");
                }
...
function showDetail(e){
	var ctext = $(e).html();
	var celem = $(e).parent().prev().find('em');
	if(ctext == "查看完整回复"){
		$(e).html("收起");
		celem.eq(0).hide();
		celem.eq(1).show();
...
function redPacket(){
    ...
    $('.red-packet-box').on('click',function(){
        if(_uzw.user.userid){
            //已登录
            rpPop.removeClass('hide');
            _uzw.ui.mask.show();
        }else{
             _uzw.iframe.pop(_uzw.domain.u + "/QuickLoginV1?actionName=_up.collect.cancle");
        }
...
//即时产品切位加载
function ajaxImmCutCheck(pid, godate, sflag){
	$.ajax({
	    async:false,
	    //dataType: "jsonp",  
	    //jsonp: "callback",
	    url: "//buy.uzai.com/ashx/ashxImmCutCheck.ashx?pid=" + pid + "&godate=" + godate + "&supplierflag=" + sflag,
