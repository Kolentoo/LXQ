...
//修改合并到product.js中  lizhen begin 20161114
/*从页面中取得url*/
...
//修改合并到product.js中  lizhen end 20161114
/*smart calendar*/
function smCalendar() {
    ...
    var uztype = GetQueryString('uztype');
    //点击事件
    ...
    var cfg = {
        //修改合并到product.js中  lizhen begin 20161114
        jsonpUrl: 'http://sh.uzai.com/ashx/ashx_Calendar.ashx?pid=' + pid + '&type=1&uztype=' + uztype,
        //修改合并到product.js中  lizhen end 20161114
        isSmart: true,
        latestDate: firstDay,
...
// 新版详情页新增js
function routeNav(){
    var rd = $('#j_routeDetail');
    var routeTag = rd.find('.route-tag-nav');
    var itemList = rd.find('.item-list');
    var rtop = $('.route-cont-box1').offset().top;
    var proBook = rd.find('.product-nav-side');

    itemList.on('click',function(){
        var t = $(this);
        var ts = t.siblings();
        t.find('.nav-item').addClass('nav-item-on');
        ts.find('.nav-item').removeClass('nav-item-on');
        var tindex = t.index();
        var section = $('.route-cont-box').eq(tindex);
        var stop = section.offset().top;
        $('body,html').animate({scrollTop:stop-39},800);
    });

    $(window).scroll(function () {
        var w = $(window).scrollTop();
        var sliceLast = $('#j_products').find('.slice-mod-last');
        var sliceTop = sliceLast.offset().top;
        if (w >= rtop && w < sliceTop-39 ) {
                routeTag.addClass('route-nav');
                $('.product-nav-fixed').hide();
                proBook.removeClass('hide');
            } else {
                routeTag.removeClass('route-nav');
                $('.product-nav-fixed').show();
                proBook.addClass('hide');
            }

        $('.route-cont-box').each(function(a,b) {
            var ctop = $(this).offset().top;
            if (w>ctop-40) {
                itemList.find('.nav-item').removeClass('nav-item-on');
                itemList.find('.nav-item').eq(a).addClass('nav-item-on');
            }
        });
...
function activitySwitch(){
    var mswitch = $('.activity-infor').find('.info-bd-side');
    mswitch.on('click',function(){
        var activityMain = $('.activity-infor').find('.activity-bd-main');
        activityMain.toggleClass('switch-on');
    });
}

function proCoupon(){
    var oBtn = $('.coupon-btn').find('.p4');
...
function proBooking(){
    var bOrder = $('#JounerySpecal_0').find('.btn-order');
    bOrder.on('click',function(){
        var proIntro = $('.product-intro');
        var pintroTop = proIntro.offset().top;
        $('body,html').animate({scrollTop:pintroTop},800);
    });
}

function preferential(){
    $('.preferential-tips').on('click',function(){
        var disInforTop = $('.discount-infor').offset().top;
        $('body,html').animate({scrollTop:disInforTop-50},800);
    });
}
