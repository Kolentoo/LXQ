...
var AppLogin = (function () {
    ...
    _app.prototype.sendmobilefunc = function () {
        ...
        $.ajax({
            ...
            cache: false,
            dataType: "jsonp",
            data: { "type": "proc_one", islogin: ((_uzw.user && _uzw.user.islogin == "1")), "txtPhone": $("input[name='phonenumber']").val().trim(), "grapCode": ($("input[name='piccode']").closest("li").hasClass("hide")), "codeType": "checkCode", "validatorType": "form", "clientid": "txtValidator", "txtValidator": $("input[name='piccode']").val().trim() },
...
function orderStore() {
    ...
    popStore.on('click', '.store-item', function () {
        ...
        if ($.trim(storeAddress)) {   
            $(".store-address .s1").removeClass('hide');
            address.find('.s1').find('em').text(storeAddress);
        } else {
            $(".store-address .s1").addClass('hide');
