...
* @Last Modified time: 2017-03-14 09:59:29
*/
'use strict';
var isUrlJump = false;
$(function () {
...
function slideBarFilter() {
    ...
    //显示菜单
    nd.find('.side-sizer-bd').children('.list-item').on('mouseenter', function () {
        var o = $(this);
...
function chooseSorter() {
    ...
    jsb.find('.sub-menu').find('.list-item').on('click', function () {
        ...
        if (tag === 'price' || tag === 'date') {
            ...
            if (tag === 'price') {
                jsb.find('.menu-wrap[data-tag=date]').find('.menu-hd').find('em').text('出发日期');
                jsb.find('.menu-wrap[data-tag=date]').find('.menu-hd').find('.icon-item').addClass('icon-desc');
            } else if (tag === 'date') {
                jsb.find('.menu-wrap[data-tag=price]').find('.menu-hd').find('em').text('价格');
                jsb.find('.menu-wrap[data-tag=price]').find('.menu-hd').find('.icon-item').addClass('icon-desc');
            }
...
function generateUrl() {
    ...
    for (var para in paramarray) {
        ...
        if (paramarray[para].lastIndexOf('data-brand') != -1) {
             var shipNo = paramarray[para].split('_')[2];
            company = "cc" + getCompanyCodeByShip(paramarray[para].split('_')[1]+'_'+paramarray[para].split('_')[2]);
            ship = "cr" + shipNo;
        }
