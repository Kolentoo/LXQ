...
var BookList = {
    ...
    */
    renderAttachment: function (attachments) {
        ...
        if (attachments) {
            for (var i = 0; i < attachments.length; i++) {
                var activity = attachments[i];
                var html = $('#_attachTemplate').html();
                html = html.replace(/{attachmentName}/, activity.name );
                html = html.replace(/{attachmentDetail}/,   (activity.count)+"份" + " X " + "¥ " + (activity.price) );
                
                html = html.replace(/{attachmentAmount}/, parseFloat( (activity.price) * activity.count).toFixed(2));     
                activityHtml += html;
                 
                apTotal += (activity.price * activity.count);
            }
        }
         
        return activityHtml != '' ? $('#attachTemplate').html().replace(/{_attachHTML}/, activityHtml).replace(/{attachmentTotalAmount}/, parseFloat( apTotal).toFixed(2)) : "";
    ...
    */
    render: function (order, attachments, preActivities) {
        ...
        $("#spanPayAmountB").text("￥" + totalAmount.toString());
        return html;
    }
}