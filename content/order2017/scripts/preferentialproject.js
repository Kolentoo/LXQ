...
showError = function (obj, msg) {
    if (msg) {
        obj.closest(".jf-con").find(".label-tips").removeClass("hide");
        obj.closest(".jf-con").find(".label-tips").find("p").text(msg);
    } else {
        obj.closest(".jf-con").find(".label-tips").addClass("hide");
        obj.closest(".jf-con").find(".label-tips").find("p").text(msg);
    }
}
...
var MemberPoints = {
    usePoints: function (e) {
        ...
        $(e).parent().find(".error_entry").remove();
        $("#txtUseScore").parents(".book-code").removeClass("book-code-off");

        if (o.text() == "使用") {
            if ($.trim(txtScore.val()) == "") {
                showError($('#txtUseScore'), "请输入要使用的积分数");
                $('#txtUseScore').val('');
                $('#txtUseScore').focus();
                ...
                if (score > 0) {
                    if (score <= count) {
                        if (score % ratio == 0) {
                            ...
                            o.text("取消");
                            $("#txtUseScore").parents(".book-code").addClass("book-code-off");

                            $('#txtUseScore').removeClass('error');
                            txtScore.prop("disabled", true);
                            ...
                            var tipInfo = '当前可用<em class="font-hover" id="scoreCount">' + (count - score) + '积分</em>（<em class="font-hover">' + ratio + '积分=1元</em>）';
                            ...
                            lis.each(function () {
                                ...
                                o.find('.dis-price')
                                o.find('.dis-price p.info').html("不与" + "其他优惠" + "同享").show();
                                o.find('.dis-price').addClass("no-line");

                                o.find('.dis-price p.value').hide();
                            });
                        }
                        else {
                            //$('#pointTips').html("请输入" + ratio + "的整数倍数值");
                            showError($('#txtUseScore'), "请输入" + ratio + "的整数倍数值");
                            $('#txtUseScore').focus();
                            $('#txtHiddenUseScore').val(0);
                        }
                    }
                    else {
                        //$('#pointTips').html("输入积分数大于当前剩余积分数");
                        showError($('#txtUseScore'), "输入积分数大于当前剩余积分数");
                        $('#txtUseScore').focus();
                        $('#txtHiddenUseScore').val(0);
                    }
                }
                else {
                    //$('#pointTips').html("积分数必须大于零");
                    showError($('#txtUseScore'), "积分数必须大于零");
                    $('#txtUseScore').focus();
                    $('#txtHiddenUseScore').val(0);
                }
            }
            else {
                //$('#pointTips').html("当前线路不支持积分抵扣");
                showError($('#txtUseScore'), "当前线路不支持积分抵扣");
                $('#txtHiddenUseScore').val(0);
            }
        }
        else {
            ...
            o.text("使用");
            txtScore.prop("disabled", false);
            usedPoint = null;
            PreferentialProject.usedPoint = usedPoint;
            var a = parseInt($('#hidRemainScore').val(), 10);
            var b = parseInt($("#txtUseScore").val(), 10);
            var remainScore = a + b;
            var tipInfo = '当前可用<em class="font-hover" id="scoreCount">' + remainScore + '积分</em>（<em class="font-hover">' + ratio + '积分=1元</em>）';
            $('#pointTips').html(tipInfo);
...
var Coupons = {
    ...
    */
    chooseCoupons: function (e, couponsCode) {
        ...
        $('#txtUseVoucher').val('');
        $('#txtUseVoucher').next().text("使用");
        $('#txtUseVoucher').removeAttr("readonly");
        $('#txtUseVoucher').parents(".book-code").removeClass("book-code-off");
        $('#couponsTip').html('');
        $("#txtUseVoucher").parents(".book-code").find(".label-tips").css({ "display": "none" });

        // 如果已经选中
        if (!$(e).hasClass("check-on")) {
            PreferentialProject.selectedCoupons = null;
            // $(e).attr("class", "sale-coupon");

            $('#txtHiddenUseVoucher').val('');//优惠券隐藏域
            ...
            lis.each(function () {
                ...
                o.find('.dis-price p.info').html('').hide();
                o.find('.dis-price').removeClass("no-line");

                o.find('.dis-price p.value').show();
            });
            // 第2步，取消立减类内互斥
            ...
            // 先将其他券置为未选中状态
            $('.yh-container').find(".discount-list:not(.disabled)").each(function () {
                $(this).removeClass("discount-out");
            ...
            if (couponInfo) {
                ...
                var _selectedCoupons = new Object();
                //2,72,RY9798BBD1,500(ticketType,面值，代码，最高优惠金额)
                _selectedCoupons.code = couponInfoArr[2];
                _selectedCoupons.ticketType = couponInfoArr[0]; //1=抵扣券，2=折扣券
                _selectedCoupons.deductionAmount = couponInfoArr[3];
                _selectedCoupons.price = parseFloat(couponInfoArr[1]); //按产品方要求，优惠券价格出现小数只取整(后来又改为保留2位小数)
                _selectedCoupons.price = Coupons.getCouponsMaxAmount(_selectedCoupons);
                _selectedCoupons.suppelierID = couponInfoArr[4]; //券的供应商，是否是宝钢券
                PreferentialProject.selectedCoupons = _selectedCoupons;
                $('#txtHiddenUseVoucher').val(_selectedCoupons.code);
            }
            ...
            // 第1步，和银行的互斥
            var lis = $("input[name='preferential_" + 1 + "']").closest("li");
            lis.each(function () {
                $(this).find(".dis-price .info").html("不与" + "其他优惠" + "同享")
                $(this).find(".dis-price .info").show();
                $(this).find(".dis-price .value").hide();
                $(this).find('.dis-price').addClass("no-line");
            ...
            // 第2步，立减类内互斥
            var lis = $("input[name='preferential_" + 2 + "']").closest("li");
            lis.each(function () {
                $(this).find(".dis-price .info").html("不与" + "抵用券/红包" + "同享")
                $(this).find(".dis-price .info").show();
                $(this).find(".dis-price .value").hide();
                $(this).find('.dis-price').addClass("no-line");
            ...
            if (PreferentialProject.selectedCoupons != null && (PreferentialProject.selectedCoupons.suppelierID == 35 || PreferentialProject.selectedCoupons.suppelierID == 40)) { //宝钢优惠券-去掉已选中的标准优惠,标记为不能与宝钢优惠同享
                ...
                var lis = $("input[name='preferential_" + 3 + "']").closest("li");
                lis.each(function () {
                    var o = $(this);
                    //去掉选中的标准优惠
                    ...
                    var couponsTitle = PreferentialProject.selectedCoupons.suppelierID == 35 ? "兜礼优惠" : "国航优惠";
                    o.find('.dis-price .info').html("不与" + couponsTitle + "同享").show();
                    o.find(".dis-price .info").show();
                    o.find('.dis-price .value').hide();
                    o.find('.dis-price').addClass("no-line");
    ...
    */
    useCoupons: function (e) {
        ...
        if ((isUserTicket.val() == '0' || o.text() == "使用") && o.text() != "取消") {
            ...
            var suppelierID = Coupons.getConponsSuppelierID(v)//获取是否是宝钢券

            if (thirdPartyUserSource == 1 && suppelierID == 35) {//宝钢会员
            ...
            $.ajax({
                ...
                cache: false,
                async: false,
                url: "//buy.uzai.com/ashx/ashx0010.ashx?type=5&pId=" + pid
                + "&code=" + v
                + "&total=" + total
                + "&userid=" + userId
                ...
                success: function (msg) {
                    ...
                    // 将其他券设置为未选中状态
                    $('li.discount-list.check-on').each(function () {
                        $(this).click();
                    });
                    ...
                    if (parseInt(msg) > 0) {//验证成功
                        ...
                        selectedCoupons.price = parseFloat(msg);
                        ...
                        lis.each(function () {
                            ...
                            o.find('.dis-price p.info').html("不与" + "抵用券/红包" + "同享").show();
                        });
                        //第3步，选择了宝钢的优惠券 标准优惠互斥
                        if (PreferentialProject.selectedCoupons != null && (PreferentialProject.selectedCoupons.suppelierID == 35 || PreferentialProject.selectedCoupons.suppelierID == 40)) { //宝钢优惠券-去掉已选中的标准优惠,标记为不能与宝钢优惠同享
                            ...
                            lis.each(function () {
                                ...
                                o.removeClass("check-on");
                                var couponsTitle = PreferentialProject.selectedCoupons.suppelierID == 35 ? "兜礼优惠" : "国航优惠";
                                o.find('.dis-price p.info').html("不与" + couponsTitle + "同享").show();
                            });
                        } else {//不是宝钢优惠设置初始状态
                            // 大类互斥检测
                            if (PreferentialProject.jsonData) {
                    ...
                    txtVoucher.val(v);
                    o.text("取消");
                    txtVoucher.prop("readonly", true);
                    txtVoucher.parents(".book-code").addClass("book-code-off");
                    clickObj.attr('onclick', 'Coupons.useCoupons(this);');
                    renderBooklist(); //重新生成预订清单
                },
                error: function () {
                    o.attr('onclick', 'Coupons.useCoupons(this);');
            ...
            $('.coupons-details input:radio').attr("disabled", false);
            o.text("使用");
            txtVoucher.val('');
            txtVoucher.prop("readonly", false);
            txtVoucher.parents(".book-code").removeClass("book-code-off");
    ...
    getConponsSuppelierID: function (coupon) {
        ...
        $.ajax({
            ...
            cache: false,
            async: false,
            url: "//buy.uzai.com/ashx/AjaxCoupon.ashx?couponCode=" + coupon,
            success: function (msg) {
...
var PreferentialProject = {
    ...
    categoryMutexCheck: function (categoryId) {
        if (this.selectedActivities.length == 0) {
            for (var i = 0; i < this.jsonData.Preferentials.length; i++) {
                ...
                var lis = $("input[name='preferential_" + preferentialModel.CategoryID + "']").parent().parent('.check-disable');
                lis.each(function () {
                    var o = $(this);
                    o.removeClass('check-disable');
                    o.find('.dis-price p.info').html("").hide();
                    ...
                    if (mutexeCategorieCodes.length > 0) {
                        ...
                        lis.each(function () {
                            ...
                            if (preferentialModel.CategoryID == 1) {
                                ...
                                o.find('.dis-price p.info').html("不与" + textArr.join(",") + "同享").show();
                            }
                        });
                    }
                }
                ...
                for (var i = 0; i < preferentials.length; i++) {
                    ...
                    for (var j = 0; j < preferential.Activities.length; j++) {
                        ...
                        if (activity.Checked) {
                            result = true;
                            // // 兼容FireFox的写法
                            // var isChecked = $("input[type='checkbox'][id='" + activity.PreferentialID + "']").prop("checked");
                            // if (isChecked) {
                            //     $("input[type='checkbox'][id='" + activity.PreferentialID + "']").parent().find("label").click();
                            //     $("input[type='checkbox'][id='" + activity.PreferentialID + "']").parent().find("label").click();
                            // }
                            // else {
                            //     $("input[type='checkbox'][id='" + activity.PreferentialID + "']").parent().find("label").click();
    ...
    activitySelect: function (e, preferentialId, categoryId) {
        ...
        if (checked) {
            // $(e).removeClass("input-checkbox-box1").removeClass("input-checkbox-box");
            ...
            if (isLogin) {
                ...
                if (preferentialModel.CategoryID == 1) {
                    // 银行不与积分、优惠券同享
                    var o = $('.preferential-container').find('.dis-content');
                    ...
                    var o = $('.preferential-container').find('.dis-content .dis-topic');
                ...
                if (PreferentialProject.selectedCoupons != null && (PreferentialProject.selectedCoupons.suppelierID == 35 || PreferentialProject.selectedCoupons.suppelierID == 40)) { //宝钢优惠券-去掉已选中的标准优惠,标记为不能与宝钢优惠同享
                    if (preferentialModel.CategoryID == 3) { //选中宝钢优惠券后将选中的标准优惠活动设置为不选中
                        ...
                        $('#preferential-container').find('ul.dis-con li.check-on').each(function () {
                            $(this).click();
                        });
                    }
                }
            ...
            $(e).addClass("input-checkbox-box1").addClass("input-checkbox-box");
            // 获取该优惠活动的父类
            if (preferentialModel.InnerMutex == 0) { //类内互斥
                ...
                if (isLogin) {
                    $("#txtUseScore").removeAttr("disabled");
                    ...
                    //没登录禁用积分
                    $("#txtUseScore").attr("disabled", "disabled");
                }
            }
            setTimeout(function () {
                // 选中的优惠活动
...
try {
    PreferentialProject.jsonData = {};

    // Object.defineProperty(PreferentialProject, "jsonData", {
    //     get: function () {
    //         return JSON.parse(document.getElementById('hidActivityData').value);
    //     }
    // });
