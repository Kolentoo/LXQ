...
var filterRoute = {};
filterRoute.init = function (flag, page) {
    _util.file.load(_uzm.domain.cdnRandom() + '/content/m/v2/scripts/com/sortElements.js', function () {
        var op = $('#route-screen');
        op = (!op.get(0)) ? $('.route-screen') : op;
        if (op.get(0)) {
            $('#btnScreenSure').on('click', function () {
                ...
                tripspecial = $('#rdSpecial').prop("checked") === true ? "1" : "0";
                triphot = $('#rdHot').prop("checked") === true ? "1" : "0";
